#+TITLE: George Jones' emacs initialization du jour
#+AUTHOR: George Jones
#+EMAIL: gmj@pobox.com
#+DATE: <2016-11-19 09:29:57 Saturday>
...
**** Load packages in a sane, modern way
     - per
       ...
       it looks like you just need to maintain the list of package you
       want in the variable 'package-selected-packages, which should be
       saved in your 'custom-file when you run install-pacakge.  Move
       to new systems/installs to migrate.
...
* Configuration
  ...
  :END:
** Prerequisites
  - A recent version of emacs :: See [[https://raw.githubusercontent.com/eludom/HOWTO/master/emacsFromSrc.org][my instructions for building the
        latest emacs from xource]] (thanks to Xah lee)

  - A recent version of org mode (8.3.3 at present <2016-01-31 Sun>).
    Install either from src

    #+begin_example
    git clone git://orgmode.org/org-mode.git
    #+end_example

    or a recent version of org-plus-contrib from the archives. Add
...
*** Set up use-package to install other stuff
# #+BEGIN_SRC emacs-lisp
...
*** Abbrevs
    Load abbrevs common to all environments here.
    Abbrevs for specific environments (eg. work email addreses, etc) are loaded via
`    ~/config/*.{el,org} if neede.
**** Load Xah Lee's example abbrevs
...
**** Define a few abbrevs the hard way

     #+begin_src emacs-lisp
     (define-global-abbrev "8gj" "George Jones")
     (define-global-abbrev "8foo" "8bar")
     (define-global-abbrev "8foo" "8bar")
     (define-global-abbrev "8M" "@@html:FOO@@")
     (define-global-abbrev "8tgj" "Thanks,
---George Jones")
     (define-global-abbrev "8tg" "Thanks,
---George")
     (define-global-abbrev "8cgj" "Curious,
---George Jones")
     (define-global-abbrev "8cg" "Curious,
---George")
     (define-global-abbrev "8gf" "find . -name \\*.org -mtime -14 -type f -exec grep -i --color -nH -e  {} +")
